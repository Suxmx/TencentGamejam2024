...
namespace Framework.Audio
{
    ...
    public class AudioManager : Service,IAudioManager
    {
        #region Inspector Variables

        public string bgmRootPath = "";
        public string sfxRootPath = "";

        [Header("背景音乐属性")] [Tooltip("Is the background music mute")] [SerializeField]
        bool _musicOn = true;

        [Tooltip("背景音乐音量")] [Range(0, 1)] [SerializeField]
        ...
        [Space(3)] [Header("音效属性")] [Tooltip("The sound effects volume")] [SerializeField]
        bool _soundFxOn = true;

        [Tooltip("音效音量")] [Range(0, 1)] [SerializeField]
        ...
        [AutoService] private IAssetLoader _assetLoader;
        ...
        /// <param name="transition">How should the music change from the current to the next.</param>
        /// <param name="transition_duration">Time in secs it takes to transition.</param>
        public void PlayBGM(AudioClip clip, MusicTransition transition = MusicTransition.Swift,
        ...
        /// Only one background music can be active at a time.
        /// </summary>
        /// <param name="clipName">Path name of the target clip from the Resources folder</param>
        /// <param name="transition">How should the music change from the current to the next.</param>
        /// <param name="transition_duration">Time in secs it takes to transition.</param>
        /// <param name="volume">Playback volume.</param>
        /// <param name="pitch">Pitch level of the clip.</param>
        ...
        public void PlayBGM(string clipName, MusicTransition transition, float transition_duration, float volume,
            float pitch, float playback_position = 0)
        {
            var path = $"{bgmRootPath}/clipName";
            PlayBGM(LoadClip(path), transition, transition_duration, volume, pitch, playback_position);
        }
        ...
        {
            var path = $"{bgmRootPath}/{clipName}";
            PlayBGM(LoadClip(path), transition, transition_duration, _musicVolume, 1f);
        }
        ...
        public AudioSource PlaySFX(string clipName, float duration, float volume,
            bool singleton = false, float pitch = 1f, Action callback = null)
        {
            var path = $"{sfxRootPath}/clipName";
            ...
            host = CreateSoundFx(clip, Vector2.zero);
        ...
        /// <param name="pitch">Pitch level of the clip.</param>
        /// <param name="callback">Action callback to be invoked after the sound has finished.</param>
        public AudioSource RepeatSFX(string clipName, Vector2 location, int repeat, float volume, bool singleton = false,
            float pitch = 1f, Action callback = null)
        {
            ...
            return PlayOneShot(clipName, volume, pitch, callback);
        }
            ...
            Action callback = null)
        {
            ...
            GameObject host = CreateSoundFx(clip, Vector2.zero);
            AudioSource source = host.GetComponent<AudioSource>();
            source.loop = false;
            source.volume = _soundFxVolume * volume;
            source.pitch = pitch;
        ...
        {
            AudioClip clip = _assetLoader.Load<AudioClip>(path);
            if (clip == null)
            {
                return null;
            }
