...
namespace MirMirror
{
    ...
    {
        [Header("请配置哪组对话，请不要超出对话组数，否则报错")]
        [SerializeField]
        private int m_CurPartID;
        #region
        [SerializeField]
        ...
        public void ResetEventList()
        {
            ...
            for (int i = 0; i < m_MMDialogue_Data.m_DialoguePartDatas[m_CurPartID].m_DataEventNodes.Count; i++)
            {
                m_MMEvents.Add(new MMEvent(m_MMDialogue_Data.m_DialoguePartDatas[m_CurPartID].m_DataEventNodes[i].EventKey));
            }
        }
        ...
        private IEnumerator ParseMM_DialogueData()
        {
            ...
            m_CurrentNode = m_MMDialogue_Data.m_DialoguePartDatas[m_CurPartID].GetFirstNode();
            ...
            while (true)
            {
                switch (m_CurrentNode.DataNodeID.m_NODETYPE)
                {
                    ...
                    case NODETYPE.DIALOGUE:
                        ...
                        m_CurrentNode = m_MMDialogue_Data.m_DialoguePartDatas[m_CurPartID].GetNextDialogueNode(m_CurrentNode);
                        break;
                    case NODETYPE.CHOICE:
                        ...
                        m_CurrentNode = m_MMDialogue_Data.m_DialoguePartDatas[m_CurPartID].GetNextDialogueNode(m_CurrentNode, m_ChoiceID);
                        m_ChoiceID = -1;
                        break;
                    case NODETYPE.EVENT:
