...
public class EffectShaderGUI : ShaderGUI
{
    ...
    void DoMask()
    {
        ...
        if (mat.IsKeywordEnabled("_USEMASK_ON"))
        {
            ...
            MaterialProperty _MaskU2 = FindProperty("_MaskU2");
            editor.ShaderProperty(_MaskU2,MakeLabel(_MaskU2));
            if (_MaskU2.floatValue==0)
            {
                MaterialProperty _MaskScale = FindProperty("_MaskScale");
                editor.ShaderProperty(_MaskScale,MakeLabel(_MaskScale));
            }
            else
            {
                GUILayout.Label("遮罩强度：Custom1.z  (参数设定到0-1的范围)");
            }
        }
    ...
    void DoNoise()
    {
        ...
        if (mat.IsKeywordEnabled("_USENOISE_ON"))
        {
            ...
            {
                GUILayout.Label("扭曲强度：Custom1.w");
            }
